...
func (client *Client) QueryContainerList(request *QueryContainerListRequest) (response *QueryContainerListResponse, err error) {
	response = CreateQueryContainerListResponse()
...
func (client *Client) QueryContainerListWithChan(request *QueryContainerListRequest) (<-chan *QueryContainerListResponse, <-chan error) {
	responseChan := make(chan *QueryContainerListResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.QueryContainerList(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) QueryContainerListWithCallback(request *QueryContainerListRequest, callback func(response *QueryContainerListResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *QueryContainerListResponse
		var err error
		defer close(result)
		response, err = client.QueryContainerList(request)
...
type QueryContainerListRequest struct {
	*requests.RpcRequest
...
type QueryContainerListResponse struct {
	*responses.BaseResponse
	...
	ContainerList  []Container `json:"ContainerList" xml:"ContainerList"`
}
...
func CreateQueryContainerListRequest() (request *QueryContainerListRequest) {
	...
	request.InitWithApiInfo("cloudesl", "2020-02-01", "QueryContainerList", "", "")
