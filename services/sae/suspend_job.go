...
func (client *Client) SuspendJob(request *SuspendJobRequest) (response *SuspendJobResponse, err error) {
	response = CreateSuspendJobResponse()
...
func (client *Client) SuspendJobWithChan(request *SuspendJobRequest) (<-chan *SuspendJobResponse, <-chan error) {
	responseChan := make(chan *SuspendJobResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.SuspendJob(request)
		if err != nil {
			errChan <- err
		} else {
...
func CreateSuspendJobRequest() (request *SuspendJobRequest) {
	request = &SuspendJobRequest{
		RoaRequest: &requests.RoaRequest{},
	}
	request.InitWithApiInfo("sae", "2019-05-06", "SuspendJob", "/pop/v1/sam/job/suspendJob", "serverless", "openAPI")
