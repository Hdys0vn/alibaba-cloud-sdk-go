...
func (client *Client) SaveContactWhiteList(request *SaveContactWhiteListRequest) (response *SaveContactWhiteListResponse, err error) {
	response = CreateSaveContactWhiteListResponse()
...
func (client *Client) SaveContactWhiteListWithChan(request *SaveContactWhiteListRequest) (<-chan *SaveContactWhiteListResponse, <-chan error) {
	responseChan := make(chan *SaveContactWhiteListResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.SaveContactWhiteList(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) SaveContactWhiteListWithCallback(request *SaveContactWhiteListRequest, callback func(response *SaveContactWhiteListResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *SaveContactWhiteListResponse
		var err error
		defer close(result)
		response, err = client.SaveContactWhiteList(request)
...
type SaveContactWhiteListRequest struct {
	*requests.RpcRequest
	ContactWhiteListList  *[]string `position:"Query" name:"ContactWhiteListList"  type:"Repeated"`
	ContactWhiteListsJson string    `position:"Query" name:"ContactWhiteListsJson"`
...
func CreateSaveContactWhiteListRequest() (request *SaveContactWhiteListRequest) {
	request = &SaveContactWhiteListRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("OutboundBot", "2019-12-26", "SaveContactWhiteList", "", "")
...
func CreateSaveContactWhiteListResponse() (response *SaveContactWhiteListResponse) {
	response = &SaveContactWhiteListResponse{
