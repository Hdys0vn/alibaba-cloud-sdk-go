...
// CheckServiceLinkedRole invokes the rds.CheckServiceLinkedRole API synchronously
func (client *Client) CheckServiceLinkedRole(request *CheckServiceLinkedRoleRequest) (response *CheckServiceLinkedRoleResponse, err error) {
	response = CreateCheckServiceLinkedRoleResponse()
	err = client.DoAction(request, response)
	return
}

// CheckServiceLinkedRoleWithChan invokes the rds.CheckServiceLinkedRole API asynchronously
func (client *Client) CheckServiceLinkedRoleWithChan(request *CheckServiceLinkedRoleRequest) (<-chan *CheckServiceLinkedRoleResponse, <-chan error) {
	responseChan := make(chan *CheckServiceLinkedRoleResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.CheckServiceLinkedRole(request)
		if err != nil {
			errChan <- err
		} else {
...
// CheckServiceLinkedRoleWithCallback invokes the rds.CheckServiceLinkedRole API asynchronously
func (client *Client) CheckServiceLinkedRoleWithCallback(request *CheckServiceLinkedRoleRequest, callback func(response *CheckServiceLinkedRoleResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *CheckServiceLinkedRoleResponse
		var err error
		defer close(result)
		response, err = client.CheckServiceLinkedRole(request)
...
func CreateCheckServiceLinkedRoleResponse() (response *CheckServiceLinkedRoleResponse) {
	response = &CheckServiceLinkedRoleResponse{
