...
func (client *Client) GetAlarmLog(request *GetAlarmLogRequest) (response *GetAlarmLogResponse, err error) {
	response = CreateGetAlarmLogResponse()
...
func (client *Client) GetAlarmLogWithChan(request *GetAlarmLogRequest) (<-chan *GetAlarmLogResponse, <-chan error) {
	responseChan := make(chan *GetAlarmLogResponse, 1)
...
type GetAlarmLogRequest struct {
	*requests.RpcRequest
	AlarmItemType string           `position:"Body" name:"AlarmItemType"`
	PageNumber    requests.Integer `position:"Body" name:"PageNumber"`
	FromDate      requests.Integer `position:"Body" name:"FromDate"`
	PageSize      requests.Integer `position:"Body" name:"PageSize"`
	AlarmItemId   string           `position:"Body" name:"AlarmItemId"`
	UniqueAppId   string           `position:"Body" name:"UniqueAppId"`
	UntilDate     requests.Integer `position:"Body" name:"UntilDate"`
}
...
type GetAlarmLogResponse struct {
	*responses.BaseResponse
	...
	AlarmLogList []AlarmLog `json:"AlarmLogList" xml:"AlarmLogList"`
}
