...
func (client *Client) GetAppWithChan(request *GetAppRequest) (<-chan *GetAppResponse, <-chan error) {
	responseChan := make(chan *GetAppResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.GetApp(request)
		if err != nil {
			errChan <- err
		} else {
...
func CreateGetAppRequest() (request *GetAppRequest) {
	...
	request.InitWithApiInfo("idrsservice", "2020-06-30", "GetApp", "idrsservice", "openAPI")
