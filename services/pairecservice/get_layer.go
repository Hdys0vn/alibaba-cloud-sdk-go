...
func (client *Client) GetLayer(request *GetLayerRequest) (response *GetLayerResponse, err error) {
	response = CreateGetLayerResponse()
...
func (client *Client) GetLayerWithChan(request *GetLayerRequest) (<-chan *GetLayerResponse, <-chan error) {
	responseChan := make(chan *GetLayerResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.GetLayer(request)
		if err != nil {
			errChan <- err
		} else {
...
func CreateGetLayerRequest() (request *GetLayerRequest) {
	...
	request.InitWithApiInfo("PaiRecService", "2022-12-13", "GetLayer", "/api/v1/layers/[LayerId]", "", "")
