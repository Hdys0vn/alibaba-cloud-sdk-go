...
func (client *Client) ListAPNs(request *ListAPNsRequest) (response *ListAPNsResponse, err error) {
	response = CreateListAPNsResponse()
...
func (client *Client) ListAPNsWithChan(request *ListAPNsRequest) (<-chan *ListAPNsResponse, <-chan error) {
	responseChan := make(chan *ListAPNsResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.ListAPNs(request)
		if err != nil {
			errChan <- err
		} else {
...
func CreateListAPNsRequest() (request *ListAPNsRequest) {
	request = &ListAPNsRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("CC5G", "2022-03-14", "ListAPNs", "fivegcc", "openAPI")
